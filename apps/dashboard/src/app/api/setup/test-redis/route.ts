...
export async function POST(_request: NextRequest) {
  ...
  try {
    // Check if Redis is already installed and responding
    const existing = detectExistingRedis();
    if (existing) {
      return NextResponse.json({
        success: true,
        connectionUrl: 'redis://localhost:6379',
        message: 'Existing Redis installation detected and responding.',
        log: ['Existing Redis detected.', '> Ping test: PONG'],
        reused: true,
      });
    }
...
// ---------------------------------------------------------------------------
// Detect existing Redis
// ---------------------------------------------------------------------------

function detectExistingRedis(): boolean {
  try {
    const pong = execSync('redis-cli ping', { encoding: 'utf8', timeout: 5000, stdio: 'pipe' }).trim();
    return pong === 'PONG';
  } catch {
    // redis-cli not found or Redis not responding
    // Also try checking if the service exists but is stopped
    try {
      execSync('which redis-server', { encoding: 'utf8', stdio: 'pipe' });
      // redis-server exists but isn't responding — try to start it
      execSync('sudo systemctl start redis-server 2>/dev/null || sudo systemctl start redis 2>/dev/null', { encoding: 'utf8', timeout: 10000, stdio: 'pipe' });
      // Check again
      const pong = execSync('redis-cli ping', { encoding: 'utf8', timeout: 5000, stdio: 'pipe' }).trim();
      return pong === 'PONG';
    } catch {
